...
// Cena do Menu Principal (Login/Cadastro e Seleção de Avatar)
class MainMenuScene extends Phaser.Scene {
    ...
    create() {
        this.selectedAvatarKey = null; // Armazena a chave do avatar selecionado ('avatar_menino' ou 'avatar_menina')
        const centerX = this.cameras.main.centerX;
        const centerY = this.cameras.main.centerY;

        this.add.text(centerX, centerY - 250, 'Aventura do Saber', {
            fontSize: '48px', fill: '#FFFFFF', fontFamily: '"Poppins"'
        }).setOrigin(0.5);

        this.add.text(centerX, centerY - 150, 'Escolha seu avatar:', {
            fontSize: '24px', fill: '#DDDDDD', fontFamily: '"Poppins"'
        }).setOrigin(0.5);

        // Adiciona os avatares selecionáveis
        ...
        avatarMenino.on('pointerdown', () => {
            this.selectedAvatarKey = 'avatar_menino';
            avatarMenino.setTint(0x00ff00); // Destaque verde
            avatarMenina.clearTint();
        });

        avatarMenina.on('pointerdown', () => {
            this.selectedAvatarKey = 'avatar_menina';
            avatarMenina.setTint(0x00ff00); // Destaque verde
            avatarMenino.clearTint();
        });
...
class GameScene extends Phaser.Scene {
    ...
    init(data) {
        ...
        this.avatarKey = data.avatar || 'avatar_menino'; // Avatar padrão caso não seja passado
        this.score = data.score || 0;
    }

    preload() {
        this.load.json('questoes_mat', 'data/matematica.json');
        // Carrega a imagem do avatar que será usada na cena
